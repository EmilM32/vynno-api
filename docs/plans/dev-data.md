...
- **`./scripts/reset`** — wipe playground tables, then restore `alexdev@vynno.local` + one Identity project.
...
5. `seed` always wipes first. `alexdev@vynno.local` keeps `DefaultUserID` and bootstrap credentials so the existing SPA login still works.

### Demo accounts (after `scripts/seed`)

| Email | Password | What you see |
| --- | --- | --- |
| `alexdev@vynno.local` | `BOOTSTRAP_PASSWORD` | Power user, 7 projects (one archived), ~10 weeks of logs, one live session |
| `maya@vynno.local` | `SEED_PASSWORD` (default `local-dev-password`) | Contractor, 4 projects (one archived), idle |
| `rio@vynno.local` | same as Maya | New-ish account, two projects, short history |
...
| Bootstrap vs seed clash | Second API start creates a fourth user | `alexdev@vynno.local` keeps `DefaultUserID`; `Bootstrap` no-ops when that row has a password |
...
- [x] `scripts/reset` wipes app tables and leaves bootstrap `alexdev@vynno.local` + Identity
